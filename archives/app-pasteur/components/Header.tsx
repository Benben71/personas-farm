...
const pasteurPersonas = personas.map((p: any) => ({
  id: p.id,
  name: p.identite_profil && p.identite_profil.prenom ? p.identite_profil.prenom : p.id.charAt(0).toUpperCase() + p.id.slice(1)
}));
...
export default function Header({ projectData }: HeaderProps) {
  ...
  const navigation: Array<{
    ...
    { 
      name: 'Personas', 
      hasDropdown: true,
  dropdownItems: pasteurPersonas.map(persona => ({
        name: persona.name,
    ...
    { name: 'Stratégie scientifique', href: '/strategie' },
  ];
    ...
    <header className="sticky top-0 z-10 w-full bg-white/80 backdrop-blur-md">
      <div className="container mx-auto flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold tracking-tighter text-secondary-950">PASTEUR</h1>
        </div>
